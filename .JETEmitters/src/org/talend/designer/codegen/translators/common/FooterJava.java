...
public class FooterJava
{
  ...
  protected final String TEXT_83 = ".setProperty(\"application.name\", \"Talend Studio\");" + NL + "\t\t\tp_";
  protected final String TEXT_84 = ".setProperty(\"service.name\", \"Talend Studio Job\");" + NL + "\t\t\tp_";
  protected final String TEXT_85 = ".setProperty(\"instance.name\", \"Talend Studio Job Instance\");" + NL + "\t\t\tp_";
  ...
  protected final String TEXT_114 = ".setProperty(\"application.name\", \"Talend Studio\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_115 = ".setProperty(\"service.name\", \"Talend Studio Job\");" + NL + "\t\t\t\tproperties_";
  protected final String TEXT_116 = ".setProperty(\"instance.name\", \"Talend Studio Job Instance\");" + NL + "\t\t\t\tproperties_";
  ...
  protected final String TEXT_140 = ".class.getClassLoader().getResourceAsStream(\"config/contexts/\" + contextStr + \".properties\");" + NL + "            }" + NL + "            if (inContext != null) {" + NL + "                try {" + NL + "                    //defaultProps is in order to keep the original context value" + NL + "                    if(context != null && context.isEmpty()) {" + NL + "    \t                defaultProps.load(inContext);" + NL + "    \t                if (inOSGi && jobProperties != null) {" + NL + "                             java.util.Enumeration<String> keys = jobProperties.keys();" + NL + "                             while (keys.hasMoreElements()) {" + NL + "                                 String propKey = keys.nextElement();" + NL + "                                 if (defaultProps.containsKey(propKey)) {" + NL + "                                     defaultProps.put(propKey, (String) jobProperties.get(propKey));" + NL + "                                 }" + NL + "                             }" + NL + "    \t                }" + NL + "    \t                context = new ContextProperties(defaultProps);" + NL + "                    }" + NL + "                    if(isStandaloneMS) {" + NL + "                        // override context key-value pairs if provided using --context=contextName" + NL + "                        defaultProps.load(inContext);" + NL + "                        context.putAll(defaultProps);" + NL + "                    }" + NL + "                } finally {" + NL + "                    inContext.close();" + NL + "                }" + NL + "            } else if (!isDefaultContext) {" + NL + "                //print info and job continue to run, for case: context_param is not empty." + NL + "                System.err.println(\"Could not find the context \" + contextStr);" + NL + "            }" + NL + "            // override key-value pairs if provided via --config.location=file1.file2 OR --config.additional-location=file1,file2" + NL + "            if(isStandaloneMS) {" + NL + "                context.putAll(this.getAdditionalProperties());" + NL + "            }" + NL + "            " + NL + "            // override key-value pairs if provide via command line like --key1=value1,--key2=value2" + NL + "            if(!context_param.isEmpty()) {" + NL + "                context.putAll(context_param);" + NL + "\t\t\t\t//set types for params from parentJobs" + NL + "\t\t\t\tfor (Object key: context_param.keySet()){" + NL + "\t\t\t\t\tString context_key = key.toString();" + NL + "\t\t\t\t\tString context_type = context_param.getContextType(context_key);" + NL + "\t\t\t\t\tcontext.setContextType(context_key, context_type);" + NL + "" + NL + "\t\t\t\t}" + NL + "            }";
